...
//        self.notesTextView.text = [dateFormatter stringFromDate:self.incomeToShowDetail.recurringDateID];
        ...
        if ([self.incomeToShowDetail.recurringType isEqualToString:@"fixedAmount"]){
            ...
            if ([self.incomeToShowDetail.recurringPostiveOrNegative isEqualToString:@"negative"]) {
                self.recurringAmountLabel.text = [NSString stringWithFormat:@"-$%@", [NSNumber numberWithFloat:(self.incomeToShowDetail.recurringAmount.floatValue)]];
            } else {
